...
const SingleShipPop = ({ SingleShip, setSingleShip, orderId }) => {
    ...
    useEffect(() => {
        if (orderId !== null) {
            ...
            axios.get(`http://dev.shipease.in:8081/core-api/shipping/ship-rate-card/?order_id=${orderId}&seller_id=${sellerId}`, config)
                .then((response) => {
    ...
    const handleSubmit = (courier) => {
        axios.get(`http://dev.shipease.in:8081/core-api/shipping/ship-order/${orderId}/?courier_partner=${courier}`, {
            headers: {
                Authorization: `Bearer ${authToken}`
            }
        })
